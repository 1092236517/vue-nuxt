<template>
    <div class="main">
        ...
        <div class="left">
            <p class="hotlist">视频列表</p>
            <div v-for="news in videoList" :key="news.id" class="list" @click="inDetail(news.id)">
                <section v-if="news.cover_type == 'single'" class="infor">
                    <div class="illimg">
            ...
            <button v-if="isLoading&&videoList.length!=0" class="loading" @click="loading">加载更多</button>
            <p v-else-if="videoList.length!=0" class="noload">到底了...</p>
            <p v-else class="noload">暂无数据</p>
        </div>
        <seo></seo>
    </div>
</template>
...
export default {
    ...
    async asyncData ({app,store}) {
        let [response1, response2,response3] = await Promise.all([
            app.$request.get('/api/frontend/news/text/pc_lists',{params:{text_type:'video',per_page:10}}),
        ...
        return{
            videoList: response1.data.response.data,
    ...
    methods:{
        inforList: function(page){
           let that = this;
           this.$request.get('/api/frontend/news/text/pc_lists',{params:{text_type:'video',current_page:page,per_page:10}}).then(function(response){
                if(response.data.msg_code == 100000){
                    if(response.data.response.data.length < 10){
                        that.isLoading = false;
                    }
                    if(page == 1){
                        that.videoList = response.data.response.data;
                    }else{
                        that.videoList = that.videoList.concat(response.data.response.data);
                    }
                }else{
