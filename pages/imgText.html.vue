...
export default {
    ...
    async asyncData ({app,store}) {
        let [response1, response2,response3] = await Promise.all([
            app.$request.get('/api/frontend/news/text/pc_lists',{params:{text_type:'text',per_page:10}}),
        ...
        return{
            textList: response1.data.response.data,
    ...
    methods:{
        inforList: function(page){
           let that = this;
           this.$request.get('/api/frontend/news/text/pc_lists',{params:{text_type:'text',current_page:page,per_page:10}}).then(function(response){
